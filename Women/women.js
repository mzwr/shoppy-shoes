...
// button search
// creeat buttons
const carigories = document.querySelector('.buttons')

const displayButtons = () => {
  const buttons = [
    'all',
    ...new Set(products.map((prudect) => prudect.carigory))
  ]
 //  console.log(buttons)
 carigories.innerHTML = buttons.map((company) => {
...
carigories.addEventListener('click', (e) => {
  const el = e.target
  if (el.dataset.id === 'all'){
    filtersProduct = [...products]
  }else{
    filtersProduct = products.filter((product) => {
      return product.carigory === el.dataset.id
    })
  }
  inputSearch.value = ''
  displayProducts()
